...
const ForgotPassword = () => {
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/teacher/forgot-password', { email });
  ...
  return (
    <div className={`min-h-screen bg-gray-100 flex flex-col justify-center sm:py-12 ${darkMode ? "bg-dallas-cowboys-blue" : "bg-white"}`}>
      ...
      <div className="p-10 xs:p-0 mx-auto md:w-full md:max-w-md">
        <h1 className={`font-bold text-center text-2xl mb-5 ${darkMode ? "text-white" : "text-gray-700"}`}>Forgot Password</h1>
