...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    const success = await login('student', reg, password); // Replace with dynamic role if needed
  
    if (success) {
      const role = user?.role; // Use the user object from context
      if (role === 'student') {
        navigate('/student/'); // Redirect to admin dashboard
      } 
      setIsSuccess(true);
    } else {
      ...
      <div className="p-10 xs:p-0 mx-auto md:w-full md:max-w-md">
        ...
        <div className={`shadow w-full rounded-lg divide-y divide-gray-200 ${darkMode ? "bg-college-navy" : "bg-white"}`} >
          <form onSubmit={handleLogin} className="px-5 py-7">
            <label className={`font-semibold text-sm pb-1 block ${darkMode ? "text-white" : "text-gray-700"}`}>Reg</label>
            <input 
              type="text" 
            ...
            <div className="grid grid-cols-2 gap-1">
              <div className="text-center sm:text-left whitespace-nowrap">
                <button className="transition duration-200 mx-5 px-5 py-4 cursor-pointer font-normal text-sm rounded-lg text-gray-500 hover:bg-gray-100 focus:outline-none focus:bg-gray-200 focus:ring-2 focus:ring-gray-400 focus:ring-opacity-50 ring-inset">
                  ...
                  <a href="/student/forgot-password/"><span className="inline-block ml-1">Forgot Password</span></a>
                </button>
              </div>
            </div>
          </div>
