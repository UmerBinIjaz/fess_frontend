...
const ResetPassword = () => {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const response = await axios.post('https://fess-backend-6787.onrender.com/api/student/reset-password', { token, password });
    ...
    <div className={`min-h-screen bg-gray-100 flex flex-col justify-center sm:py-12 ${darkMode ? "bg-dallas-cowboys-blue" : "bg-white"}`}>
      ...
      <div className="py-5 text-center">
        <span className={`text-sm ${darkMode ? "text-white" : "text-gray-700"}`}>
          ...
          <a href="/student/login" className={`${darkMode ? "text-white hover:text-gray-400" : "text-blue-500 hover:text-blue-700"}`}>
            Login
          </a>
        </span>
      </div>      
