...
const GenerateAttendance = () => {
    ...
    const fetchAttendanceData = async () => {
      if (user && token) {
        try {
          setLoading(true);
          const response = await axios.post(
            `https://fess-backend-6787.onrender.com/api/admin/course/${courseId}/generate-attendance`,
            { adminId: user.id },
  ...
  const handleExport = async () => {
    try {
        const response = await axios.post(
            `https://fess-backend-6787.onrender.com/api/admin/course/${courseId}/export-attendance`,
            { adminId: user.id },
      ...
      <div className={`max-w-7xl mx-auto p-6 ${darkMode ? "bg-college-navy" : "bg-white text-gray-900"}`}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div className='flex justify-start'>
            <h3 className="m-0 font-medium text-base text-blue-500"> Attendance Report</h3>
          </div>
