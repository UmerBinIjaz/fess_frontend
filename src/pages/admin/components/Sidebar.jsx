...
const Sidebar = ({ isSidebarOpen }) => {
  ...
  const [listOBEdropdownOpen, setlistOBEDropdownOpen] = useState(true);
  ...
  const listOBEtoggleDropdown = () => {
    setlistOBEDropdownOpen(!listOBEdropdownOpen);
  };  
  ...
  const isadminCoursePage = location.pathname.includes(`/admin/course/${courseId}`);
  ...
  // Check if current URL matches the admin course page
  // const isadminCoursePage = window.location.href.includes(`/admin/course/${courseId}`);
            ...
            <li className="inline-block relative">
              ...
              <div className={`menu ${dropdownOpen ? 'block' : 'hidden'}`} role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabIndex="-1">
                ...
                <div className="py-1" role="none">
                  <Link to="/admin/profile" className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"> <FaUserCircle className="mr-3 text-xl" /> My Profile </Link>
                  <Link to="/admin/edit-profile" className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"> <FaEdit className="mr-3 text-xl" /> Edit Profile </Link>
                  <Link to="/admin/logout" className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"> <FaEnvelope className="mr-3 text-xl" /> Logout </Link>
                </div>
              </div>
            </li>
          </ul>
          ...
          <div className={`md:flex md:flex-col md:items-stretch md:opacity-100 md:relative md:mt-4 absolute top-0 left-0 right-0 z-40 overflow-y-auto overflow-x-hidden h-auto items-center flex-1 rounded ${collapseShow} ${darkMode ? "bg-college-navy !important text-white" : "bg-white text-gray-900"}`}>
            ...
            <ul className="rounded-md z-10 w-full">
              <li className="items-center">
                <Link to="/admin/"   className={`text-xs uppercase py-3 font-bold block ${darkMode ? location.pathname === "/admin/" ? "text-blue-300" : "text-white" : getActiveClass("/admin/")}`}> Dashboard </Link>
              </li>
            </ul>

            <ul className="rounded-md z-10 w-full">
              <li className="items-center">
                <Link to="/admin/students" className={`text-xs uppercase py-3 font-bold block ${darkMode ? location.pathname === "/admin/students" ? "text-blue-300" : "text-white" : getActiveClass("/admin/students")}`}> Students </Link>
              </li>
            </ul>

            <ul className="rounded-md z-10 w-full">
              <li className="items-center">
                <Link to="/admin/candidates" className={`text-xs uppercase py-3 font-bold block ${darkMode ? location.pathname === "/admin/candidates" ? "text-blue-300" : "text-white" : getActiveClass("/admin/candidates")}`}> Candidates </Link>
              </li>
            </ul>

            <ul className="rounded-md z-10 w-full">
              <li className="items-center">
                <Link to="/admin/result" className={`text-xs uppercase py-3 font-bold block ${darkMode ? location.pathname === "/admin/result" ? "text-blue-300" : "text-white" : getActiveClass("/admin/result")}`}> Results </Link>
              </li>
            </ul>
